...
-(void)customImageView
{
    ...
    //imageView = [[UIImageView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH/2)-49,0, 98, 28)];
    imageView = [[UIImageView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH/2)-36.6,0, 73.5, 21)];

    imageView.backgroundColor = [UIColor clearColor];
    imageView.animationImages = imageArray;
    imageView.center = CGPointMake(SCREEN_WIDTH/2.0f, superBounds.size.height/2.0);
    [self addSubview:imageView];
    [imageView setAnimationDuration:1.5];
    
    UILabel *label = [[UILabel alloc]init];
    label.frame = CGRectMake(10, 0, SCREEN_WIDTH-20, 15);
    label.backgroundColor = [UIColor clearColor];
    label.textColor = RGB_COLOR_String(@"aeaeae");
    label.textAlignment = NSTextAlignmentCenter;
    label.text = @"加载中...";
    label.font = [UIFont systemFontOfSize:14];
    [self addSubview:label];
    label.center = CGPointMake(SCREEN_WIDTH/2.0f, superBounds.size.height/2.0+22);
