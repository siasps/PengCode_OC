...
- (void)initSubviews{
    ...
    [forgetButton setTitleColor:ThemeColor forState:UIControlStateNormal];
    forgetButton.backgroundColor = [UIColor clearColor];
    [self.view addSubview:forgetButton];
    [forgetButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(loginBtn.mas_bottom).mas_offset(20);
...
//    let para = ["apikey":"1.5","apisecret":"checked","UserNo":"13032155538","UserPassword":sign] as [String : Any];
//
//    ZHttpTools.shareInstance.request(methodTypea: RequestType.POST, urlString: "", parametersa:para, progress: { (progress : Progress) in
//
//    }) { (result, error) in
...
//    }
    ...
    [HHttpManager POST:@"http://www.future-plus.com.cn/api/login.php" parameters:dict success:^(id responseObject) {
        ...
        [MBProgressHUD showAlterWithMessage:@"网络异常，请稍后重试!"];
...
////获取自助收银权限
//- (void)getCashLimitWithCache:(BOOL)cache{
//
//    if (!cache) {
//        [HMDataRequest deleteCacheWithUrl:Server_order_is_cash];
//    }
//
//    NSString *supplier_id = emptyString([[NSUserDefaults standardUserDefaults]objectForKey:HMSupplier_id]);
//    NSString *city_id = emptyString([[NSUserDefaults standardUserDefaults]objectForKey:HMVenue_cityid]);
//
//
//    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
//    [dict setObject:city_id forKey:@"city_id"];
//    [dict setObject:supplier_id forKey:@"supplier_id"];
//
//
//    [HMDataRequest getRequestWithUrl:Server_order_is_cash withParams:dict withCacheTime:0 showProgressInView:self.view CallBack:^(id responseObject, id error) {
//
//        if (error==nil) {
//            NSDictionary *respondDict = responseObject;
//            NSDictionary *infoMap= [respondDict objectForKey:@"infoMap"];
...
//            if ([flag isEqualToString:@"1"]) {
//
//                [[NSUserDefaults standardUserDefaults] setObject:emptyString(infoMap[@"is_cash_site_supplier"]) forKey:HMSupplier_is_cash];
//                [[NSUserDefaults standardUserDefaults] setObject:emptyString(infoMap[@"is_cash"])
//                                                          forKey:HMSupplier_join_cash];
...
//
//            } else {
//                NSString * reasonStr = [infoMap objectForKey:@"reason"];
//                [MBProgressHUD showAlterWithMessage:reasonStr];
//            }
//
//        } else{
//            NIF_TRACE(@"%@",error);
